...
@section('title', 'Termék címke szerkesztése')

@section('content')
    <div class="container mt-lg-4 mt-3 mb-150">
        @include('admin.webshop.partials.alerts')

        <div class="row"><div class="col-lg-12"><h2 class="header-box my-2">Termék címke szerkesztése: {{ $label->name }}</h2></div></div>

        <form method="POST" action="{{ route('admin.webshop.labels.update', $label) }}">
            @csrf
            @method('PUT')
            <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="color" class="form-control" id="bg_color" name="bg_color" value="{{ old('bg_color', $label->bg_color) }}">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="text_color">Szövegszín</label>
                                    <input type="color" class="form-control" id="text_color" name="text_color" value="{{ old('text_color', $label->text_color) }}">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-lg-6 mb-3">
                    ...
                    <div class="content-box bordered text-center py-5 mb-3">
                        <div id="label-preview" style="display: inline-block; padding: 10px 20px; font-size: 20px; font-weight: bold; border-radius: 4px; background-color: {{ $label->bg_color }}; color: {{ $label->text_color }};">
                            {{ $label->name }}
                        </div>
                    </div>
                </div>
            ...
            <div class="text-center mt-3">
                ...
                <button type="submit" class="btn btn-primary font-weight-bold ml-2">Címke frissítése</button>
            </div>
        </form>
    </div>
